...
function GenuineDeal() {

    const [genuineDeal, setGenuineDeal] = useState([]);

    useEffect(()=>{
        axios.get('/api/residential-property/genuine-deal').then(res=>{
            if(res.status == 200){
                setGenuineDeal(res.data.genuine_deal);
            }
    ...
    return (
        <>
        <PageBanner Banner={DkBanner} MobileBanner={MbBanner} PageName="GENUINE DEAL" />
        <div className="max-w-7xl mx-auto md:px-16 px-4 pt-8 pb-20">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {genuineDeal && genuineDeal.map((property)=>(
