...
function BestProperty() {
    ...
    const [bestProperty, setBestProperty] = useState([]);

    useEffect(()=>{
        axios.get('/api/residential-property/best-property').then(res=>{
            if(res.status == 200){
                setBestProperty(res.data.best_property);
            }
        }).catch(console.log("Best Property Error"));
    },[]);

    return (
        <>
        <PageBanner Banner={DkBanner} MobileBanner={MbBanner} PageName="BEST PROPERTY" />
        <div className="max-w-7xl mx-auto md:px-16 px-4 pt-8 pb-20">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {bestProperty && bestProperty.map((property)=>(
